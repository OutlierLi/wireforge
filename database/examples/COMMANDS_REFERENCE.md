...
> 自动生成于 2026-07-02 21:28:47 +0800；源文件 [`console/commands.json`](../../console/commands.json)。
...
CSG 2016 与 DLT645-2007 对**协议地图中未定义的 DI/AFN 变体**不会整帧解码失败：

1. **基本帧结构照常解析**（长度、控制域、AFN/SEQ/DI 或 645 地址/功能码/DI 等）
2. **未注册 payload 以 raw hex 输出**，路由路径中出现 `raw_remaining`
3. **`/build from-frame`** 可原样重建或 `--set` 修改 raw 字段（CSG：`di_payload`；645：`di_data`）

示例：

```text
/decode proto csg hex 68 10 00 40 03 01 FF FF 03 E8 AA BB CC DD 3B 16
/build from-frame --from_frame "68 10 00 40 03 01 FF FF 03 E8 AA BB CC DD 3B 16" --set di_payload=11223344
```

帧界/校验和错误，或 CSG 传输方向+地址域标志无法匹配时仍会失败。详见 [`AGENTS.md`](../../AGENTS.md#unknown-variant--raw-payload未识别变体)。
