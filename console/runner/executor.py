...
from console.runner.context import RunContext, StepRecord
...
class StepExecutor:
    ...
    def resolve_step(self, step: dict[str, Any], ctx: RunContext) -> dict[str, Any]:
    ...
    def _execute(self, step: dict[str, Any], ctx: RunContext) -> dict[str, Any]:
        ...
        result = runtime.execute(command, command_args)
        if result.get("status") != "success":
            raise StepFailed(str(step["id"]), action, result)
        return result
    ...
    def _prepare_request_args(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if build is not None:
            ...
            build_result = runtime.execute("build", dict(build_args))
    ...
    def _scope(self, ctx: RunContext) -> dict[str, Any]:
        scope = dict(ctx.vars)
        scope.update(ctx.step_results)
        return scope
