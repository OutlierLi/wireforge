#!/usr/bin/env python3
"""DLT645-2007 请求-响应配对全量 Build/Decode 测试。

用法:
  python3 scripts/run_dlt645_full_pair_test.py
  python3 scripts/run_dlt645_full_pair_test.py --pair-id read_data_0001ff00
  python3 scripts/run_dlt645_full_pair_test.py --fail-fast -q
"""
...
from tests.protocol_build_utils import MessageTestResult, run_dlt645_pair_message
from tests.protocol_info import DLT645_FIELD_DEFAULTS

LOG_ROOT = _PROJECT_ROOT / "log" / "dlt645_full_test"
...
def run_all_pairs(
    *,
    ...
    """执行全量配对测试，写入 log/dlt645_full_test/<run_id>/。"""
    pairs_data = load_dlt645_pairs()
    missing = validate_downlink_coverage(pairs_data)
    if missing:
        raise SystemExit(f"pairs yaml missing downlink entries: {missing}")

    compile_protocol(REGISTRY, "dlt645_2007", output_dir=COMPILED_DIR)
    ir = ProtocolIR.from_json_file(f"{COMPILED_DIR}/dlt645_2007.ir.json")
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        ...
        emit("=" * 70)
        emit(f"  DLT645 全量配对 Build/Decode 测试  run_id={run_id}")
        emit("=" * 70)
        ...
        for pair in pairs_data["pairs"]:
            ...
            for msg in iter_pair_messages(pair):
                result = run_dlt645_pair_message(
                    msg, ir, build_engine, decode_engine, defaults,
                )
                ...
                if msg.di:
                    extra = f" DI={msg.di}"
                label = f"{pid}/{msg.slot} FUNC={msg.func}{extra} {msg.dir}"
            ...
            all_serial_lines.append(f"chain: {chain}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="DLT645 配对全量 build/decode 测试")
